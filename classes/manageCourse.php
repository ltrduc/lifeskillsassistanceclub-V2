...
class manageCourse
{
    ...
    public function setCourse($subjects, $group, $period, $local, $dates, $semester, $schoolyear, $teacher)
    {
        ...
        $query = "SELECT * FROM `tbl_course` WHERE `subjects` = '$subjects' AND `group` = '$group' 
        AND `period` = '$period' AND `local` = '$local' AND `dates` = '$dates' 
        AND `semesters` = '$semester' AND `schoolyear` = '$schoolyear' AND `teacher` = '$teacher'";
